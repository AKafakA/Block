...
parallel-ssh -t 0 -h block/config/hosts "pip install dacite orjson"
...
parallel-ssh -t 0 -h block/config/ampere_hosts "sudo nvidia-smi -mig 0"
...
parallel-ssh -t 0 -h block/config/ampere_hosts "git clone ${BLOCK_GITHUB_LINK} && cd Block && git checkout ${BRANCH_NAME} && pip install -r requirements.txt"
...
# 5. Pin transformers to 4.x for compatibility with older P100 vLLM branch
# The P100 vLLM (cara_p100_v_6.0) is incompatible with transformers 5.x
parallel-ssh -t 0 -h block/config/pascal_hosts "sudo pip install transformers==4.57.6"
