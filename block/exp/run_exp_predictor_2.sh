...
parallel-ssh -i -t 0 -h block/config/hosts "cd Block && export PYTHONPATH=. && nohup python block/prediction/predictor/api_server.py --config_path $CONFIG_PATH --metric_type $METRIC_TYPE --enable_time_estimation $ENABLE_TIME_ESTIMATION --batch_size_cap $BATCH_CAP --workers $NUM_WORKERS $APPEND_CHUNKED_PREFILL --threshold_batch_size_for_time_estimation $BATCH_SIZE_THRESHOLD_FOR_TIME_ESTIMATION --predictor_index 2 --port 8300 --predictor_timeout $PREDICTOR_TIMEOUT $APPEND_CPU_TRACKING > experiment_output/logs/predictor_2.log 2>&1 &"