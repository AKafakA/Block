...
# Fix Ubuntu mirror (us.archive.ubuntu.com can be unreachable from CloudLab) and enable universe repo
...
parallel-ssh -t 0 -h $HOST_FILE "sudo apt install -y python3-pip python3-venv ccache ninja-build"
...
parallel-ssh -t 0 -h $HOST_FILE "git clone ${VLLM_GITHUB_LINK} && cd vllm && sudo VLLM_USE_PRECOMPILED=1 pip install --editable . && sudo chown -R \$(whoami) ~/vllm"
...
parallel-ssh -t 0 -h $HOST_FILE "pip install flashinfer-python -i https://flashinfer.ai/whl/cu124/torch2.6/"
parallel-ssh -t 0 -h $HOST_FILE "pip install triton==3.2.0 transformers==4.50.3 ninja orjson"

# A100 specific: NVMe mount + env vars
