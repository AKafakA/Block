...
BLOCK_HOST_CONFIG="block/config/a100_8x7b_host_configs_16pred.json"
PREDICTOR_CONFIG="block/config/llama7b_a100_config.json"

# Global scheduler settings
NUM_INSTANCES=8
NUM_PREDICTORS_PER_INSTANCE=16
...
deploy_vllm() {
    ...
    log "Waiting 360s for all vLLM instances to load models and capture CUDA graphs..."
    sleep 360
...
deploy_predictors() {
    ...
    # Start all predictors on both nodes (16 per GPU, 64 per node, 128 total)
    # Deploy in batches of 8, sleep 10s between batches to avoid OOM
    # Port mapping: skip 8200 (global scheduler)
    # GPU0->8100-8115, GPU1->8300-8315, GPU2->8400-8415, GPU3->8500-8515
    PREDICTOR_BASE_PORTS="8100 8300 8400 8500"
    for host in "$NODE0_HOST" "$NODE1_HOST"; do
        log "Starting 64 predictors on $host (16 per GPU, batches of 8)..."
        ...
        log "Waiting 30s before next node..."
        sleep 30
    done

    sleep 30
    log "All 128 predictors deployed"
}
